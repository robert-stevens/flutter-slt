...
class Topic {
  ...
  Topic({this.id, this.authorId, this.username, this.avatar, this.title, this.description, this.count, this.dateTime,});
  ...
  static Future<List<Topic>> getTopics(String keyword, int startCount, int incrementCount) async {

    final String token = await User.getToken();

    startCount = startCount ~/ incrementCount;
    
    // print('url: ${Constants.WORDPRESS_URL+'mobile/v2/topics?s=$startCount&i=$incrementCount&k=$keyword'}');

    final http.Response response = 
      await http.get(Constants.WORDPRESS_URL+'mobile/v2/topics?s=$startCount&i=$incrementCount&k=$keyword', 
        headers: {
